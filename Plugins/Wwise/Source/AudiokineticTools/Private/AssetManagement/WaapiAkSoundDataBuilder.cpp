...
void WaapiAkSoundDataBuilder::OnSoundBankGenerated(uint64_t Id, TSharedPtr<FJsonObject> ResponseJson)
{
	...
	if (ResponseJson->HasField(WwiseWaapiHelper::BANK_INFO))
	{
...

			{
				...
				AllParseTask.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, RequiredData, PlatformName, Language, ResponseJson, InfoJson]
				{
					if (RequiredData->AkAudioEvent && RequiredData->EventPlatformData)
					{
						...
						bChanged |= sharedThis->ParseSoundBankInfo(RequiredData->AkAudioEvent, RequiredData->EventPlatformData, PlatformName, Language, InfoJson, false);
		...
		else if (AkAuxBusIt)
		{
			...
			{
				FScopeLock AutoLock(&ParseTasksLock);
				AllParseTask.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, AkAuxBus, PlatformName, ResponseJson, InfoJson]
				{
					if (AkAuxBus)
					{
						...
						if (AuxBusPlatformData)
						{
							...
							bChanged |= sharedThis->ParseSoundBankInfo(AkAuxBus, AuxBusPlatformData, PlatformName, FString(), InfoJson, false);
...

			{
				...
				AllParseTask.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, RequiredData, PlatformName, Language, ResponseJson, InfoJson]
				{
					if (RequiredData->AkAudioBank && RequiredData->AudioBankPlatformData)
					{
						...
						bChanged |= sharedThis->ParseSoundBankInfo(RequiredData->AkAudioBank, RequiredData->AudioBankPlatformData, PlatformName, Language, InfoJson, false);
		...
		else if (SoundBankName == AkInitBankName)
		{
			{
				FScopeLock AutoLock(&ParseTasksLock);
				AllParseTask.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, PlatformName, ResponseJson, InfoJson, InitBank = AkAssetDatabase.InitBank]
				{
					...
					if (InitBankPlatformData)
					{
						...
						bChanged |= sharedThis->ParseSoundBankInfo(InitBank, InitBankPlatformData, PlatformName, FString(), InfoJson, true);
